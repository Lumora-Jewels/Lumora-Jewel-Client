...
export const cartService = {
  ...
  getCart: async (userId: string): Promise<Cart> => {
    return cartApi.get(`/api/cart/${userId}`);
  },

  // Add item to cart
  addItem: async (itemData: AddToCartRequest): Promise<Cart> => {
    return cartApi.post('/api/cart', itemData);
  },

  // Update cart item
  updateItem: async (cartId: string, itemId: string, updateData: UpdateCartItemRequest): Promise<Cart> => {
    return cartApi.put(`/api/cart/${cartId}/item/${itemId}`, updateData);
  },

  // Remove item from cart
  removeItem: async (cartId: string, itemId: string): Promise<Cart> => {
    return cartApi.delete(`/api/cart/${cartId}/item/${itemId}`);
  },

  // Clear entire cart
  clearCart: async (userId: string): Promise<void> => {
    return cartApi.delete(`/api/cart/clear/${userId}`);
  },
};
